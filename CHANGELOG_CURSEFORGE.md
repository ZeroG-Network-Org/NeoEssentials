...
### Added
- **2026-09-08** — Build 68 ([`24a11344`](https://github.com/ZeroG-Network-PTY-LTD/NeoEssentials/commit/24a11344bcdfc1c4018dd420c775a142ce21de05)) — Fixed doubled rank prefixes in Discord — NeoEssentials' own Discord messages (chat/join/leave/mute/AFK/advancement/PMs, all three adapters) now resolve the prefix from the permission system directly instead of the vanilla scoreboard team, so they can never double regardless of a bridge mod's own native relay. Verified live against a real Discord server.
- **2026-09-07** — Build 67 ([`b853001d`](https://github.com/ZeroG-Network-PTY-LTD/NeoEssentials/commit/b853001d1e9f48e11397e9fc3ee7a6f6c7bc39a8)) — Wider Discord event coverage: Mc2Discord now relays AFK/private messages and reads its avatar URL from config instead of hardcoding it; DCIntegration now relays advancement/mute/AFK when routed to an explicit Discord channel (all additive-only, confirmed safe against DCIntegration's own native advancement relay via bytecode).
- **2026-09-07** — Build 66 ([`58480e2a`](https://github.com/ZeroG-Network-PTY-LTD/NeoEssentials/commit/58480e2a25c545bf7593eb942c26421f59f6d85b)) — SDLink/Mc2Discord's native-relay conflict warnings (previously console-only) are now also exposed via the dashboard's Discord status API, for admins configuring integration through it.
- **2026-09-07** — Build 65 ([`4b681a5b`](https://github.com/ZeroG-Network-PTY-LTD/NeoEssentials/commit/4b681a5bd3e21106927730e0b8e66fc74a786e64)) — Rich Discord embeds for SDLink's join/leave/mute/AFK/advancement events when routed to an explicit Discord channel — each now has its own `discordEmbedTemplate.<event>` override with sensible defaults, matching what chat messages already got. Set an event's `enabled` to `false` to keep plain text for just that event.
- **2026-09-07** — Build 64 ([`287fbdc2`](https://github.com/ZeroG-Network-PTY-LTD/NeoEssentials/commit/287fbdc2f1f9b026961bda590982bbbb226b9c2e)) — SDLink's native-relay conflict detection now parses `simple-discord-link.toml` with a real TOML parser instead of a best-effort text scan; long chat/PM/mute/AFK/advancement text sent to Discord is now truncated before sending across all three bridge-mod adapters.
- **2026-09-07** — Build 63 ([`4aafc239`](https://github.com/ZeroG-Network-PTY-LTD/NeoEssentials/commit/4aafc2394cdcf761b2ff50b2244d8f7f20352164)) — Discord role → permission group sync — new `discordrolesync.json` maps Discord role IDs to permission groups, synced via DCIntegration (highest-priority group wins on multiple matches, re-checked on a timer and on join, only grants/upgrades, never demotes, internal permission manager only).
- **2026-09-07** — Build 62 ([`03a6f764`](https://github.com/ZeroG-Network-PTY-LTD/NeoEssentials/commit/03a6f764c7e8ec7af335001bec1c86c9a0918d35)) — Mc2Discord native relay conflict detection — NeoEssentials now reads `config/mc2discord.toml` on startup and checks whether Mc2Discord's own channel subscriptions already relay chat/join/leave/advancement events natively, suppressing its own default-route send for that event type to avoid posting it twice. Explicit per-channel overrides still always send.
