...
public final class ShopNpcEntity {
    ...
    public static Entity create(Level level, UUID shopId, String shopName, String entityTypeId, boolean aiEnabled) {
        ...
        if (entity == null) {
            ...
            entity = new ArmorStand(EntityTypes.ARMOR_STAND, level);
        }
        ...
        CompoundTagCompat.putUUID(entity.getPersistentData(), NBT_SHOP_ID, shopId);
        return entity;
    }
     ...
     * {@link EntityTypes#ARMOR_STAND} if the id is blank, unknown, or unparsable — the same
    ...
    public static EntityType<?> resolveEntityType(String entityTypeId) {
        if (entityTypeId == null || entityTypeId.isBlank()) return EntityTypes.ARMOR_STAND;
        Identifier loc = Identifier.tryParse(entityTypeId);
        if (loc == null) {
            NeoLog.warn(LOGGER, LogCategory.GENERAL,
                "Invalid entity type id '{}' for a shop NPC, falling back to armor_stand", entityTypeId);
            return EntityTypes.ARMOR_STAND;
        }
        return BuiltInRegistries.ENTITY_TYPE.getOptional(loc).orElseGet(() -> {
            NeoLog.warn(LOGGER, LogCategory.GENERAL,
    ...
    public static boolean isValidEntityTypeId(String id) {
        ...
        return loc != null && BuiltInRegistries.ENTITY_TYPE.getOptional(loc).isPresent();
    }
    ...
    public static UUID getShopId(Entity entity) {
        return CompoundTagCompat.getUUID(entity.getPersistentData(), NBT_SHOP_ID);
    }

    /** Returns {@code true} if this entity is a NeoEssentials shop NPC. */
    public static boolean isShopNpc(Entity entity) {
        return CompoundTagCompat.hasUUID(entity.getPersistentData(), NBT_SHOP_ID);
    }
}
